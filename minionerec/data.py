    ...
    def __init__(self, item_file, index_file, tokenizer=None, max_len=2048, sample=-1, test=False, seed=0, category=""):
        ...
        for item_id, sids in self.indices.items():
            if item_id in self.item_feat:
                ...
                # Keep every Semantic-ID token, including a collision suffix.
                if len(sids) >= 3:
...
class RLSeqTitle2SidDataset(CSVBaseDataset):
    ...
    def get_history(self, row):
        # Prefer the original title sequence when available.  The MM-OneRec
        # atomic-to-SID CSV intentionally contains only SID fields, so use
        # those as a faithful fallback for RL smoke/training.
        if 'history_item_title' in row.index:
            ...
            inter_titles = ", ".join(history_item_sid)
    ...
    def _convert_to_semantic_ids(self, item_ids):
        ...
        for item_id in item_ids:
            ...
            if item_id_str in self.indices:
                ...
                if len(sids) >= 3:
                    # Keep every token so deduplicated SIDs remain unique.
                    combined_sid = "".join(sids)
