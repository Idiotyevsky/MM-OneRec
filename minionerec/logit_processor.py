    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for batch_id, beam_sent in enumerate(input_ids.view(-1, self._num_beams, input_ids.shape[-1])):
            for beam_id, sent in enumerate(beam_sent):
                ...
                # Beam search may call processors once more after a sequence
                # has emitted EOS. Keep the beam terminated instead of
                # reporting a spurious missing Trie prefix.
                if self.eos_token_id is not None and int(sent[-1]) == int(self.eos_token_id):
                    mask[row_index, self.eos_token_id] = 0
                    continue
                if self.count == 0:
