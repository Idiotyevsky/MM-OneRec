...
# Support `python scripts/evaluate.py ...` from the repository root.
...
def main(
    ...
    val_dataset = EvalSidDataset(train_file=test_data_path, tokenizer=tokenizer, max_len=2560, category=category, test=True, K=K, seed=seed, sample=sample)
    ...
    for i in range(len(test_data)):
        ...
        # Keep the legacy fields and add the common metrics schema used by
        # scripts/mm/evaluate_metrics.py.
        test_data[i]["target"] = str(test_data[i].get("output", "")).strip()
        test_data[i]["predictions"] = [str(value).strip() for value in test_data[i].get("predict", [])]
    result_path = Path(result_json_data)
    result_path.parent.mkdir(parents=True, exist_ok=True)
    # The processor is recreated for each generation batch; aggregate the
    # observable counters in the result metadata for invalid-prefix diagnosis.
    result_metadata = {
        "constraint": "trie_prefix",
        "num_beams": num_beams,
        "max_new_tokens": max_new_tokens,
        "sample": sample,
    }
    if outputs:
        result_metadata["note"] = "completed EOS beams are held at EOS by the Trie processor"
    result_metadata["catalog_size"] = len(info_semantic)
    for row in test_data:
        row["evaluation_metadata"] = result_metadata
